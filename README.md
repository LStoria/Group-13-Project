...
*   Nhấp **Register** nếu muốn tạo tài khoản mới, hoặc **Login** để đăng nhập trực tiếp. Giao diện ứng dụng sẽ chuyển hướng dựa trên vai trò tài khoản được phân quyền.
...
1.  **Xác thực và Phân quyền người dùng (Security & Roles)**:
    ...
    *   Phân quyền người dùng chặt chẽ trên Server thành 3 nhóm quyền: `BIDDER`, `SELLER`, và `ADMIN`.
2.  **Đăng bán sản phẩm kèm hình ảnh (Seller Upload)**:
    *   Seller đăng sản phẩm trực tiếp từ giao diện đồ họa.
    *   Chọn ảnh minh họa từ máy tính cục bộ thông qua `FileChooser`.
    *   Mã hóa dữ liệu ảnh nhị phân thành chuỗi văn bản **Base64** để truyền trực tiếp qua Socket TCP và lưu trữ an toàn trong SQLite Database.
    *   Client tự động giải mã Base64 ngược thành ảnh để hiển thị trên giao diện người dùng.
...
4.  **Đồng bộ đếm ngược & Gia hạn tự động (Server-side Countdown & Anti-Sniper)**:
    ...
    *   Cơ chế **Anti-Sniper**: Nếu có lượt đặt giá hợp lệ trong **15 giây cuối cùng** của phiên đấu giá, Server tự động reset thời gian còn lại của sản phẩm đó về **15 giây** và thông báo tới toàn hệ thống, tạo môi trường cạnh tranh lành mạnh.
